<script lang="ts">
  ...
  import { ModalDetail, ModalReporting, ModalVote } from "../modal";
  ...
  let isOpenModalReporting: boolean = false;
  let iconVote: string = "";
...
<Masonry
  ...
  idKey={"_id"}
  {minColWidth}
  maxColWidth={items.length < 4 ? 330 : 800}
  gap={15}
  animate={true}
  duration={500}
>
  <div
    in:fade
    out:fade
    class="bg-white rounded-2xl shadow-md hover:scale-[1.02] transition duration-200 cursor-pointer"
  >
    <section
      on:click={() => {
        isOpenModal = true;
...
{#if isOpenModal}
  <ModalDetail
    bind:isOpenModal
    bind:isOpenModalVote
    bind:isOpenModalReporting
    bind:dataModal
  />
{/if}

<script lang="ts">
	...
	import { Swiper, GridContent, Searcher, Submenu, ModalCreate } from '$ui/components';
					...
					items = [
						...
						...data.hits.map((e: any, index: number) => {
							return {
								...
								img: imageRd[Math.round(Math.floor(Math.random() * (imageRd.length - 0 + 0) + 0))],
								title: 'ทำไมไม่มีเรื่องรัฐประหารในหนังสือประวัติศาสตร์ไทย'
							};
						})
					];
...
<section class="mt-8 text-center">
	...
	<P class="mb-8" align="center" size="sm" space="normal" weight="medium" opacity={1}
		>ช่วยกันตรวจสอบความบ้งของการศึกษาไทย</P
	>
